    ...
    <el-table :data="filterTableData" style="width: 100%">
      ...
      <el-table-column align="right">
        ...
        <template #default="scope">
          <el-button plain @click="genMutantFLLogByModelName(scope.$index, scope.row)">定位报告生成</el-button>
          <el-button @click="getMutantFLLogByModelName(scope.$index, scope.row)">定位报告展示</el-button>
          <!--          <el-button plain @click="ElMessageBox.alert('对抗检测')">对抗检测</el-button>-->
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="dialogTableVisible" title="定位结果展示" style="width: 70%">
      <el-table :data="gridData" height="400">
        <el-table-column property="mutant_name" label="变异名称" width="auto" show-overflow-tooltip />
        <el-table-column property="model_faults" label="真实故障" width="auto" show-overflow-tooltip />
        <el-table-column property="log" label="定位报告" width="auto" show-overflow-tooltip />
      </el-table>
    </el-dialog>
  </div>
</template>
...
const genMutantFLLogByModelName = async (index: number, row: DLModel) => {
  ...
  // 发送请求
  await axios
    .post(`/section3Api/genMutantFLLogByModelName`, {
      body: {
        name: row.name
      }
    })
    .then(async (res) => {
      ...
      if (msg.includes("start")) {
        ...
        await ElMessageBox.alert(`已存在故障报告文件！\n${res.data.data}`, "警告", {
          config: undefined,
          confirmButtonText: "确认",
          type: "warning"
        });
...
const getMutantFLLogByModelName = async (index: number, row: DLModel) => {
  console.log("row", row.name, typeof row.name);
  ...
  // 发送请求
  await axios
    .post(`/section3Api/getMutantFLLogByModelName`, {
      body: {
        name: row.name
      }
    })
    .then(async (res) => {
      ...
      if (res.data.flag == "fail") {
        ElMessageBox.alert("未检测到故障报告文件，请先生成！", "警告", {
          config: undefined,
          confirmButtonText: "确认",
          type: "error"
        });
